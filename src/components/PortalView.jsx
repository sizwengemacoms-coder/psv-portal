...
export default function PortalView({ jobs, ads, onSwitchToAdmin }) {
  ...
  // Mobile job detail: full-screen slide-up sheet
  if (selectedJob) {
    ...
    return (
      <div style={s.detailScreen}>
        ...
        <div style={s.detailBody}>
          ...
          <p style={s.detailDept}>{selectedJob.department}</p>
          ...
          <div style={s.infoGrid}>
            ...
            <div style={s.infoBlock}><p style={s.infoLabel}>Closing Date</p><p style={s.infoValue}>{selectedJob.closing}</p></div>
          </div>

          <div style={s.section}>
            <p style={s.sectionLabel}>Minimum Requirements</p>
          ...
          <div style={s.applyBox}>
            ...
            <p style={s.sectionText}>Submit a fully completed Z83 application form with an updated CV to the advertising department. Quote reference <strong>{selectedJob.ref}</strong>.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={s.page}>
      ...
      {bannerAd && (
        <div style={{ ...s.banner, background: bannerAd.color }}>
          <div>
            ...
            <p style={s.bannerSub}>{bannerAd.subtitle}</p>
          </div>
      ...
      <div style={s.searchBar}>
        ...
        <button onClick={() => setShowFilters(v => !v)} style={{ ...s.filterToggleBtn, background: activeFilterCount > 0 ? "#2563EB" : "#F5F5F7", color: activeFilterCount > 0 ? "#fff" : "#1C1C1E" }}>
          ...
          <div style={s.filterGrid}>
            <div style={s.filterItem}>
              ...
              <div style={s.selectWrap}>
                <select style={s.select} value={filterCategory} onChange={e => { setFilterCategory(e.target.value); setSelectedJob(null); }}>
            ...
            <div style={s.filterItem}>
              ...
              <div style={s.selectWrap}>
                <select style={s.select} value={filterLevel} onChange={e => { setFilterLevel(e.target.value); setSelectedJob(null); }}>
            ...
            <div style={s.filterItem}>
              ...
              <div style={s.selectWrap}>
                <select style={s.select} value={filterCircular} onChange={e => { setFilterCircular(e.target.value); setSelectedJob(null); }}>
        ...
        {filteredJobs.length === 0 ? (
          <div style={s.emptyState}>
            ...
            <p style={{ fontSize: 13, color: "#8E8E93" }}>
              {jobs.length === 0 ? "Upload a circular in the Admin dashboard to get started." : "Try adjusting your filters."}
            </p>
          </div>
        ) : (
          filteredJobs.map(job => {
            ...
            return (
              <article key={job.postNo} onClick={() => setSelectedJob(job)} style={s.card}>
                <div style={s.cardTop}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={s.cardDept}>{job.department}</p>
                  </div>
                  <i className="ti ti-chevron-right" style={{ fontSize: 16, color: "#C7C7CC", flexShrink: 0, marginLeft: 8, marginTop: 4 }} />
                </div>
                <div style={s.cardFoot}>
                  <span><i className="ti ti-map-pin" style={{ marginRight: 4 }} />{job.centre.split(":").pop().trim()}</span>
                  <span><i className="ti ti-calendar" style={{ marginRight: 4 }} />Closes {job.closing}</span>
                </div>
              </article>
            );
          })
...
const s = {
  ...
  adminBtn: { width: 36, height: 36, borderRadius: 10, background: "rgba(0,0,0,0.05)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", color: "#48484A", flexShrink: 0 },
  ...
  bannerCta: { background: "rgba(255,255,255,0.2)", border: "none", borderRadius: 6, padding: "5px 10px", color: "#fff", fontWeight: 600, fontSize: 11 },
  ...
  clearBtn: { position: "absolute", right: 8, background: "none", border: "none", color: "#8E8E93", display: "flex", alignItems: "center" },
  filterToggleBtn: { height: 40, minWidth: 40, borderRadius: 10, border: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 4, paddingInline: 12, fontWeight: 600, fontSize: 13, position: "relative" },
  ...
  clearAllBtn: { marginTop: 12, background: "none", border: "1px solid #E5E5EA", borderRadius: 8, padding: "6px 14px", fontSize: 13, color: "#636366", fontWeight: 500 },
  ...
  card: { background: "#fff", borderRadius: 14, padding: "14px", border: "1px solid #F0F0F0", boxShadow: "0 1px 3px rgba(0,0,0,0.04)", cursor: "pointer", WebkitTapHighlightColor: "transparent", active: { background: "#F5F5F7" } },
  ...
  backBtn: { width: 36, height: 36, borderRadius: 10, background: "rgba(0,0,0,0.05)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", color: "#1C1C1E" },
