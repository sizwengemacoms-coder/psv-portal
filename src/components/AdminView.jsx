...
export default function AdminView({ jobs, setJobs, ads, setAds, onViewPortal }) {
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      const circularNo = headerNum ? parseInt(headerNum[1]) :
                         fileNum   ? parseInt(fileNum[1])   :
                         circulars.length > 0 ? Math.max(...circulars)+1 : 1;

      setStatus("Parsing job listings…");
          ...
          <div>
            ...
            {showAdForm && (
              <form onSubmit={saveAd} style={a.formCard}>
                ...
                <input required placeholder="Headline" value={newAd.title}
                  onChange={e=>setNewAd(p=>({...p,title:e.target.value}))} style={a.input} />
                <input placeholder="Subline" value={newAd.subtitle}
                  onChange={e=>setNewAd(p=>({...p,subtitle:e.target.value}))} style={a.input} />
                <input placeholder="CTA label" value={newAd.cta}
                  onChange={e=>setNewAd(p=>({...p,cta:e.target.value}))} style={a.input} />
                <div style={a.selectWrap}>
                  <select value={newAd.position}
                    onChange={e=>setNewAd(p=>({...p,position:e.target.value}))} style={a.select}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,margin:"10px 0"}}>
                  ...
                  <input type="color" value={newAd.color}
                    onChange={e=>setNewAd(p=>({...p,color:e.target.value}))}
                    style={{width:40,height:32,border:"1px solid #E5E5EA",borderRadius:6}} />
                </div>
